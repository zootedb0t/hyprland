...
# YouTube helper (play / download)
# Dependencies: yt-dlp, rofi, wl-clipboard, mpv, libnotify

TERM="foot"
ROFI_THEME="$HOME/.config/rofi/yt.rasi"
...
get_url() {
	...
	[ -z "$url" ] && {
		notify-send "YouTube" "No URL in clipboard"
		exit 1
	}
	echo "$url"
}
...
menu() {
	printf "Play Audio\nPlay Video\nDownload Video\n" |
		rofi -dmenu -i -p "YouTube" -theme "$ROFI_THEME" 2>/dev/null
}
...
"Download Video")
	"$TERM" -e sh -c "
			...
			notify-send 'YouTube' 'Download failed'
		"
	;;
esac
